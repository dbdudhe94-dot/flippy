...
import com.droid.flippy.hid.HidKeyboardActivity
import com.droid.flippy.plugin.PluginIcons
...
/**
 * Display name for an Other-screen section key. The keys themselves stay
 * stable (including the legacy Russian "ПРОЧЕЕ"/"ПЛАГИНЫ" constants) so
 * plugin section matching keeps working — only the header text is
 * localized.
 */
@Composable
fun localizedSectionTitle(section: String): String = when (section) {
    OtherSections.INFRARED -> stringResource(R.string.other_section_infrared)
    OtherSections.BLUETOOTH -> stringResource(R.string.other_section_bluetooth)
    OtherSections.OTHER -> stringResource(R.string.other_section_other)
    OtherSections.PLUGINS -> stringResource(R.string.other_section_plugins)
    else -> section
}
...
fun functionDestinationSections(): List<Pair<String, List<FunctionDestination>>> {
    ...
    sections += OtherSections.INFRARED to listOf(
        FunctionDestination(Icons.Default.Computer, stringResource(R.string.ir_flipper_remotes), stringResource(R.string.other_ir_flipper_desc) + " + " + stringResource(R.string.ir_cat_tv), "other/ir_flipper_home", OtherSections.INFRARED),
    ...
    sections += OtherSections.BLUETOOTH to listOf(
        ...
        FunctionDestination(Icons.Default.Chat, "Flippy Chat", stringResource(R.string.other_flippy_chat_desc), "other/flippy_chat_global", OtherSections.BLUETOOTH),
    ...
    sections += OtherSections.OTHER to listOf(
        ...
        FunctionDestination(Icons.Filled.WifiOff, stringResource(R.string.network_hub_title), stringResource(R.string.network_hub_card_description), "other/network_diagnostic_hub", OtherSections.OTHER),
