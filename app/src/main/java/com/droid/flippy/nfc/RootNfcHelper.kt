...
object RootNfcHelper {
    ...
    /** Shell-level elevation (Shizuku/Dhizuku, no root) — enough for `svc nfc`. */
    fun hasShizuku(): Boolean = try {
        com.droid.flippy.shizuku.ShizukuManager.isReady()
    } catch (_: Throwable) {
        false
    }

    /** True when NFC can be force-enabled (root) or already toggled (Shizuku). */
    fun hasElevated(): Boolean = hasRoot() || hasShizuku()
