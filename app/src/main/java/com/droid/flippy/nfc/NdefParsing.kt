...
fun summarizeNdef(
    message: NdefMessage,
    emptyContent: String = "Empty NDEF message",
    ...
    if (records.isEmpty()) {
        return NdefSummary(recordType = "Empty", content = emptyContent, rawHex = rawHex)
    }
    ...
    for (record in records) {
        val (type, content) = decodeRecord(record, decodeErrorPrefix)
        types += type
...
private fun decodeRecord(record: NdefRecord, decodeErrorPrefix: String): Pair<String, String> {
    return try {
        when {
        ...
        "Unknown" to ("$decodeErrorPrefix: ${e.message ?: e.javaClass.simpleName}\npayload=${record.payload.toHexString(sep = " ")}")
    }
}
