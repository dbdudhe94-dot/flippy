...
object UserIrRemoteStore {
    ...
    /** Appends learned buttons to an existing remote (used by Learn mode). */
    fun appendCommands(context: Context, id: Int, commands: List<IrButton>): Boolean {
        ensureLoaded(context)
        if (commands.isEmpty()) return false
        val index = remoteList.indexOfFirst { it.id == id }
        if (index < 0) return false
        remoteList[index] = remoteList[index].copy(commands = remoteList[index].commands + commands)
        persist(context)
        return true
    }
...
fun IRFlipperHome(navController: NavController) {
    ...
    val tvTitle = stringResource(R.string.ir_cat_tv)
    ...
    val catRows = buildList {
        add(Triple(Icons.Default.Tv, tvTitle, "other/ir_tv_home"))
        ...
        LazyColumn(
            ...
            item {
                M3SegmentedListItem(
                    index = 0,
                    count = 1,
                    headline = stringResource(R.string.ir_learn_title),
                    supporting = stringResource(R.string.ir_learn_hint),
                    leadingIcon = Icons.Default.Add,
                    leadingIconTint = MaterialTheme.colorScheme.primary,
                    onClick = { navController.navigate("other/ir_learn") },
                )
            }
            ...
            SectionTopBar(
                ...
                actions = {
                    ...
                    if (editMode) {
                        ...
                        FlippyIconButton(
                            onClick = {
                                val id = selectedSingle ?: return@FlippyIconButton
...
fun IRJammerScreen(navController: NavController? = null) {
    ...
    LaunchedEffect(Unit) {
        val result = withContext(Dispatchers.IO) {
            ...
            val desktop = File("C:/Users/loush/projects/Flippy/Flipper/IRjammer.ir")
                                    ...
                                    while (kotlinx.coroutines.currentCoroutineContext().isActive && isSending) {
                                        ...
                                        for ((index, btn) in list.withIndex()) {
                                            ...
                                            withContext(Dispatchers.Main) {
                                                ...
                                                statusMessage = context.getString(R.string.ir_jammer_sending_signals) +
                                                    context.getString(R.string.ir_jammer_cycle, cycle)
                                            }
                                            delay(8)
                                        }
                                    }
