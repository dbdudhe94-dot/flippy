...
class PluginDeviceApis(private val context: Context, private val pluginId: String) {
    ...
    fun irStatusJson(): String {
        return try {
            val usbState = runCatching { com.droid.flippy.ir.usb.UsbIrManager.state.value }
                .getOrNull()
            JSONObject()
                .put("stormRunning", try { com.droid.flippy.IrWidgetRuntime.isStormRunning() } catch (_: Throwable) { false })
                .put("jammerRunning", try { com.droid.flippy.IrWidgetRuntime.isJammerRunning() } catch (_: Throwable) { false })
                .put("hasConsumerIr", context.packageManager.hasSystemFeature("android.hardware.consumerir"))
                .put("hasBuiltInEmitter", try { com.droid.flippy.IrBackend.hasBuiltInEmitter(context) } catch (_: Throwable) { false })
                .put("usbIrOpen", usbState?.isOpen == true)
                .put("usbIrConnected", usbState?.connectedLabel ?: "")
                .put("usbIrProtocol", usbState?.connectedProtocol ?: "")
                .put("usbIrRoute", usbState?.route?.name ?: "AUTO")
                .put("usbIrSupportedPresent", usbState?.devices?.any { it.isSupported } == true)
                .toString()
        } catch (_: Exception) {
            "{}"
        }
    ...
    fun irToggleStorm(): Boolean = try {
        try { com.droid.flippy.IrWidgetRuntime.toggleStorm() } catch (t: Throwable) {
    ...
    fun irToggleJammer(): Boolean = try {
        try { com.droid.flippy.IrWidgetRuntime.toggleJammer() } catch (t: Throwable) {
    ...
    fun httpDownload(
        ...
        val base = java.io.File(context.filesDir, "flippy_plugins_data/$pluginId").apply { mkdirs() }.canonicalFile
        ...
        com.droid.flippy.plugin.PluginDownloadPolicy.request(pluginId, 1, maxBytes) { allowed ->
            if (!allowed) {
                mainHandler.post {
    ...
    fun rootExecScript(script: String, callback: (String) -> Unit) {
        ...
        rootCmd(
            "TMP=/data/local/tmp/flippy_plugin_$$.sh; " +
