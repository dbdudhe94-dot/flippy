package com.droid.flippy.plugin.bridge
...
class PluginAndroidApis(
    ...
    private val filesDir by lazy {
        File(appCtx.filesDir, "flippy_plugins_data/$pluginId").apply { mkdirs() }
    }
    ...
    fun clipboardSet(text: String): Boolean = try {
        val cm = appCtx.getSystemService(Context.CLIPBOARD_SERVICE) as ClipboardManager
        cm.setPrimaryClip(ClipData.newPlainText("flippy", text))
        true
    } catch (_: Exception) {
        false
    }
    ...
    fun notifyShow(id: Int, title: String, text: String, channelId: String = "flippy_plugins"): Boolean {
        return try {
            val nm = appCtx.getSystemService(Context.NOTIFICATION_SERVICE) as NotificationManager
            if (Build.VERSION.SDK_INT >= 26) {
                val ch = NotificationChannel(channelId, "Flippy Plugins", NotificationManager.IMPORTANCE_DEFAULT)
    ...
    fun irHasEmitter(): Boolean {
        return runCatching { com.droid.flippy.IrBackend.isAvailable(appCtx) }.getOrDefault(false)
    }

    fun irCarrierFrequenciesJson(): String {
        return try {
            val arr = JSONArray()
            com.droid.flippy.IrBackend.carrierFrequencies(appCtx).forEach { r ->
    ...
    fun irTransmit(freqHz: Int, pattern: IntArray): Boolean {
        return try {
            if (freqHz <= 0 || pattern.isEmpty() || pattern.any { it <= 0 }) return false
            val button = com.droid.flippy.IrButton(
                name = null,
                frequency = freqHz,
                pattern = pattern
            )
            com.droid.flippy.IrBackend.transmit(appCtx, button)
        } catch (e: Exception) {
    ...
    // ---- USB IR dongles (see com.droid.flippy.ir.usb.UsbIrManager) ----

    fun usbIrDescribeJson(): String {
        return runCatching { com.droid.flippy.ir.usb.UsbIrManager.describeJson() }
            .getOrDefault("{\"open\":false,\"devices\":[]}")
    }

    fun usbIrRequestPermission(vendorId: Int, productId: Int): Boolean {
        return try {
            val usb = appCtx.getSystemService(Context.USB_SERVICE) as? UsbManager ?: return false
            val target = usb.deviceList.values.firstOrNull {
                it.vendorId == vendorId && it.productId == productId
            } ?: return false
            com.droid.flippy.ir.usb.UsbIrManager.requestPermission(target)
            true
        } catch (e: Exception) {
            Log.w(TAG, "usbIrRequestPermission", e)
            false
        }
    }
