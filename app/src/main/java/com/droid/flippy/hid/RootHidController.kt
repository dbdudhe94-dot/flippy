        ...
        return try {
            ...
            val configured = suShell(
                "if [ -f /sys/class/android_usb/android0/state ]; then " +
                    ...
                    "elif [ -f /sys/kernel/config/usb_gadget/flippy_hid/UDC ]; then " +
                    "U=\$(cat /sys/kernel/config/usb_gadget/flippy_hid/UDC 2>/dev/null); " +
    ...
    fun setupHidGadget(): Boolean {
        return try {
            ...
            val commands = """
                ...
                echo "Flippy" > strings/0x409/manufacturer
                echo "Flippy HID Device" > strings/0x409/product
