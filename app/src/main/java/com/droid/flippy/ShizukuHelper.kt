...
object ShizukuHelper {
    ...
    fun runShellCommandWithOutput(command: String): String =
        runShellCommandWithOutput(command, timeoutSec = 30L)
    ...
    fun runShellCommandWithOutput(command: String, timeoutSec: Long): String {
        return try {
            ...
            try {
                ...
                var code = try {
                    if (process.waitFor(timeoutSec.coerceAtLeast(1L), TimeUnit.SECONDS)) {
                        process.exitValue()
                    } else {
                        Log.w(TAG, "command timed out after ${timeoutSec}s: $command")
                        try {
