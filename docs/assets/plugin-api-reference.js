const sdkText = {
  en: {
    ...
    unitReturn: 'No return value.', nodeReturn: 'A JSON-compatible native UI node rendered by Flippy.',
    ...
    nfc: 'Requires NFC hardware and an enabled adapter. Tag methods need a tag delivered to Flippy while the plugin is active.',
    ...
    shizuku: 'Requires Shizuku to be running and permission granted to Flippy.',
    ...
    dex: 'Runs trusted native code in the Flippy process. Verify SHA-256, keep payloads private and release every resource during unload.',
  ...
  ru: {
    ...
    unitReturn: 'Значение не возвращается.', nodeReturn: 'JSON-совместимый узел нативного интерфейса, который отрисовывает Flippy.',
    ...
    nfc: 'Нужны NFC и включённый адаптер. Для методов метки Flippy должен получить Tag, пока плагин активен.',
    ...
    shizuku: 'Shizuku должен быть запущен, а Flippy должно быть выдано разрешение.',
    ...
    dex: 'Запускает доверенный нативный код в процессе Flippy. Проверяйте SHA-256, храните нагрузку приватно и освобождайте все ресурсы при выгрузке.',
