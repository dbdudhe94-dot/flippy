...
messages.en.settingsContribBody = 'api.registerSettings renders header, switch, slider, nav and card entries directly in Flippy Settings.'
...
messages.en.stormScopeBody = 'It intercepts both ir_storm and other/ir_storm. Returning None on every other route preserves ordinary remotes, TV Be Gone and IR Jammer. Remove the route check to intercept every Flippy IR transmission.'
messages.en.stormModify = 'Change frequency or pattern, then let Flippy continue sending.'
...
messages.en.hookCatalogLead = 'Every screen route, named action, service operation and event currently emitted by Flippy.'
messages.en.hookCatalogRuleTitle = 'Call and intercept are different'
messages.en.hookCatalogRuleBody = 'The Android and Device API lists contain functions a plugin can call. Only the entries below can be intercepted. Registering hook_action("*") does not expose functions that Flippy never dispatches.'
...
messages.en.infraredAvailableWhere = 'When Flippy checks whether any IR transmitter is available.'
...
messages.en.newHookBody = 'It cannot currently be intercepted through the public plugin hook API. Flippy must add invokeActionHooks(name, payload) or invokeServices(service, operation, payload) at that Kotlin call site before plugin authors can hook it.'
...
messages.ru.settingsContribBody = 'api.registerSettings отрисовывает header, switch, slider, nav и card непосредственно в настройках Flippy.'
...
messages.ru.stormScopeBody = 'Он перехватывает ir_storm и other/ir_storm. Возврат None на остальных маршрутах сохраняет работу обычных пультов, TV Be Gone и ИК Глушилки. Уберите проверку маршрута, чтобы перехватывать все ИК-отправки Flippy.'
messages.ru.stormModify = 'Изменить частоту или паттерн, а затем продолжить штатную отправку Flippy.'
...
messages.ru.hookCatalogLead = 'Все экранные маршруты, именованные действия, сервисные операции и события, которые сейчас выдаёт Flippy.'
messages.ru.hookCatalogRuleTitle = 'Вызвать и перехватить — разные возможности'
messages.ru.hookCatalogRuleBody = 'Списки Android и Device API содержат функции, которые плагин может вызвать. Перехватить можно только элементы каталога ниже. Регистрация hook_action("*") не открывает функции, которые Flippy не отправляет в диспетчер.'
...
messages.ru.infraredAvailableWhere = 'Когда Flippy проверяет наличие любого ИК-передатчика.'
...
messages.ru.newHookBody = 'Сейчас её нельзя перехватить через публичный API хуков. В соответствующее место Kotlin-кода Flippy нужно добавить invokeActionHooks(name, payload) или invokeServices(service, operation, payload), после чего плагинмейкеры смогут её перехватывать.'
...
messages.en.settingsContribTitle = 'Native plugin settings'
messages.en.settingsContribBody = 'api.registerSettings adds Material 3 Expressive sections directly to Flippy Settings. Switches and sliders persist in the plugin store and emit setting_changed; nav and card items open plugin screens.'
...
messages.en.currentPointsBody = 'Flippy exposes shared infrared transmission plus Bluetooth section selection, All actions, BLE modes, Advert presets and every outgoing BLE advertising packet. The complete exact list is in the hook catalog.'
...
messages.en.metaPermissions = 'Capabilities shown before installation; Flippy also detects direct API usage in the source'
...
messages.en.installLead = 'Flippy handles .dolphyplugin files and shows a compact, partially expanded Material sheet before any code runs. Installation closes the sheet and confirms success with a Flippy toast.'
...
messages.ru.settingsContribTitle = 'Нативные настройки плагина'
messages.ru.settingsContribBody = 'api.registerSettings добавляет разделы Material 3 Expressive прямо в настройки Flippy. Переключатели и ползунки сохраняются в хранилище плагина и отправляют setting_changed, а nav и card открывают экраны плагина.'
...
messages.ru.currentPointsBody = 'Flippy открывает общую отправку ИК, выбор раздела Bluetooth, действия All, режимы BLE, пресеты Advert и каждый исходящий BLE advertising-пакет. Полный точный список приведён в каталоге хуков.'
...
messages.ru.bleModeApiLead = 'register_ble_mode добавляет режим плагина рядом со штатными режимами BLE с такой же карточкой, активным состоянием, общей задержкой и действием «Остановить всё». Flippy отправляет ble_mode_start, ble_mode_stop и ble_mode_delay_changed только плагину-владельцу. Циклом advertising управляет сам плагин; его обязательно нужно остановить при stop и unload.'
messages.ru.assetApiTitle = 'Загрузка и применение ассетов плагина'
messages.ru.assetApiLead = 'download_assets запрашивает все файлы одним пакетом. Каждый путь ограничен приватной папкой плагина, maxBytes задаёт предел ответа, а sha256 проверяет файл. Если подтверждение не отключено в «Настройки плагинов → Безопасность», Flippy до начала передачи показывает имя плагина, число файлов и заявленный общий размер. apply_asset импортирует сохранённый файл в штатный редактор Bad USB, пользовательские ИК-пульты или пользовательский HTML QR Audio Spoofer.'
...
messages.ru.metaPermissions = 'Возможности, показываемые до установки; Flippy также обнаруживает прямые вызовы API в исходнике'
...
messages.ru.installLead = 'Flippy открывает .dolphyplugin и до запуска кода показывает компактный частично свёрнутый Material-экран. После установки экран закрывается, а результат подтверждается тостом Flippy.'
...
messages.en.dexTrustBody = 'A loaded DEX runs inside the Flippy process with the app permissions. Always publish the SHA-256, verify it before loading and never execute an untrusted downloaded payload.'
messages.en.dexEmbeddedTitle = 'Embed and load a raw DEX'
messages.en.dexEmbeddedLead = 'Store the raw DEX as Base64 in the .dolphyplugin file. Flippy removes whitespace, decodes it, verifies SHA-256 and creates the class loader. Raw DEX uses InMemoryDexClassLoader on Android 8+; older Android and ZIP-based JAR/APK payloads use a private read-only code-cache file and DexClassLoader.'
...
messages.en.exportAppClassBody = 'Publish a Flippy or dependency class by its fully qualified name.'
...
messages.en.sourceBridgeTitle = 'Expose Flippy internals from open source'
...
messages.ru.dexTrustBody = 'Загруженный DEX выполняется внутри процесса Flippy с разрешениями приложения. Всегда публикуйте SHA-256, проверяйте его до загрузки и никогда не запускайте недоверенную скачанную нагрузку.'
messages.ru.dexEmbeddedTitle = 'Встраивание и загрузка сырого DEX'
messages.ru.dexEmbeddedLead = 'Сохраните сырой DEX в Base64 внутри .dolphyplugin. Flippy удалит пробелы, декодирует данные, проверит SHA-256 и создаст загрузчик. На Android 8+ сырой DEX использует InMemoryDexClassLoader; старые версии Android и ZIP-контейнеры JAR/APK используют приватный read-only файл кодового кеша и DexClassLoader.'
...
messages.ru.exportAppClassBody = 'Публикует класс Flippy или зависимости по полному имени.'
...
messages.ru.sourceBridgeTitle = 'Открытие внутренних классов Flippy по исходникам'
...
messages.ru.dexLimitsBody = 'Размер .dolphyplugin может достигать 48 МиБ, а каждый декодированный DEX/JAR/APK — 32 МиБ. При отключении, удалении или обновлении библиотеки Flippy сначала автоматически отключает активные зависимые плагины; включите потребителей снова после готовности библиотеки. Android не умеет выгружать Class после использования кода. Flippy удаляет ссылки на загрузчики и экспорты при остановке владельца, но полностью гарантированная выгрузка после обновления возможна только перезапуском процесса. Делайте имена экспортов глобально уникальными и закрывайте потоки, receiver, сокеты и оборудование в on_plugin_unload.'

const pageText = {
  home: {
    en: ['Flippy — Wireless toolkit for Android', 'Flippy is an expressive Android toolkit for Bluetooth, infrared, NFC, HID, networks and Python plugins.'],
    ru: ['Flippy — беспроводные инструменты для Android', 'Flippy — выразительный Android-набор для Bluetooth, ИК, NFC, HID, сетей и Python-плагинов.']
  },
  docs: {
    en: ['Flippy Plugin Documentation', 'Documentation for creating Python .dolphyplugin extensions for Flippy.'],
    ru: ['Документация плагинов Flippy', 'Документация по созданию Python-расширений .dolphyplugin для Flippy.']
  }
}

const storedLanguage = localStorage.getItem('flippy-language')
...
function setLanguage(nextLanguage) {
  ...
  localStorage.setItem('flippy-language', language)
}
